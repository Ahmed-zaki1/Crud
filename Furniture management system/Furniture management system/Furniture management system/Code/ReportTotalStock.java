...
import java.sql.*;
	 ...
	 public class ReportTotalStock extends Furniture implements ActionListener
	{
String h[]={"Furniture ID","Furniture Name","Company","Type","Quantity","Unit Price"};
		int cnt=0,i=0;
		//Label l1;
		JScrollPane jpane;
		JTable t1;
...
Connection c=DriverManager.getConnection("jdbc:mysql://localhost:3306/Furniture","root","");
			...
			String str="select * from FurnitureStock";
			...
			String s="select * from FurnitureStock";
			rs=st.executeQuery(s);
			
			
			while(rs.next())
			{
				record1[i][0]=rs.getString("FurnitureID");
				record1[i][1]=rs.getString("FurnitureName");
				record1[i][2]=rs.getString("CompanyNm");
				record1[i][3]=rs.getString("Type");
				record1[i][4]=rs.getString("Qty");
				record1[i][5]=rs.getString("UnitPrice");
